...
export function init() {
  ...
  if (shutdownCommand) {
    ...
    console.log('== Idle timeout set to ' + idleTimeoutSeconds + ' seconds');
    if (! (idleTimeoutSeconds > 0)) {
      console.log('== No idle timeout value, idle timeout is disabled');
    }
  } else {
    idleTimeoutSeconds = NaN;   // No shutdown command available
    console.log('== No shutdown command, idle timeout is disabled');
  }
}
...
function parseInterval(str: string) {
  ...
  str = str.replace(/\s*/, '');
  if (str) {
    console.log('== garbage left over in interval string: ' + str);
    return NaN;
  }
  return total;
}
...
function shouldResetOnPath(path: string) {
  console.log('== shouldReset sees path=' + path);
  if (path.indexOf('/_timeout')==0) {
    console.log('== path ' + path + ' should not reset idle timeout');
    return false;
  } else {
    return true;
  }
...
export function setupResetOnWebSocketRequests(socket : net.Socket) {
  socket.on('data', (data:any) => {
    ...
    console.log('== Got data event on socket, typeof(data)=' + typeof(data));
    console.log(data);
    console.log('== length = ' + data.length);
    if (isHeartbeat(data)) {
      console.log('== Got a heartbeat, this does not reset the idle timer');
    } else {
      reset();
    }
  });
...
function hasTimedOut() {
  if (isTimeoutEnabled()) {
    ...
    console.log('== idleSeconds=' + Math.ceil(10*idleSeconds)/10 + ', idleTimeoutSeconds=' + idleTimeoutSeconds);
...
function shutdownIfTimedOut() {
  if (hasTimedOut()) {
    ...
    try {
      console.log('== Idle timeout, shutting down with this command: ' + shutdownCommand);
      childProcess.execSync(shutdownCommand, {env: process.env});
      console.log('== Shutdown command succeeded');
    } catch (err) {
...
function postTimeoutHandler(userId: string, request: http.ServerRequest, response: http.ServerResponse): void {
  ...
  request.on('end', function() {
    ...
    console.log('== got form data, type=' + typeof(formData));
