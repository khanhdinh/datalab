define(['base/js/dialog', 'base/js/events'], function(dialog, events) {
  ...
  function setupKernelBusyHeartbeat() {
    events.on('kernel_busy.Kernel', function() {
      debugLog('== kernel_busy event received');
      runBusyTimer();
    });
    events.on('kernel_idle.Kernel', function() {
      debugLog('== kernel_idle event received');
      clearBusyTimer();
      maybeResetIdleTimeout();
    });
  }
  ...
  function setupDisconnectHandler() {
    events.on('kernel_connection_failed.Kernel', function() {
      debugLog('== Disconnected; timeoutInfo:');
      debugLog(timeoutInfo);
      alertIfTimedOut();
    });
  }

  function alertIfTimedOut() {
    if (timeoutInfo.enabled && timeoutInfo.idleTimeoutSeconds > 0) {
      ...
      if (secondsRemaining <=1 ) {
        ...
        window.setTimeout(alertIdleShutdown, delayToLetOtherHandlersRun);
      }
    }
  ...
  function alertIdleShutdown() {
    debugLog('== Idle timeout exceeded');
    const shutdownMsg = ('The datalab server shut down after exceeding the idle timeout of ' +
        secondsToString(timeoutInfo.idleTimeoutSeconds));
    dialog.modal({
  ...
  function updateTimeoutDisplay(dropdown) {
    if (!dropdown) {
      debugLog('== updateTimeoutDisplay no dropdown');
      return;
    }
    debugLog('== updating timeout display');
    debugLog(dropdown);
    if (timeoutInfo.enabled) {
      ...
      const roundedSecondsRemaining = roundToApproximateTime(secondsRemaining);
      const maybeAbout = (roundedSecondsRemaining != secondsRemaining)?'about ':'';
      const details = (roundedSecondsRemaining == 0) ?
        'Idle timeout exceeded' :
        'Idle timeout in ' + maybeAbout + secondsToString(roundedSecondsRemaining);
      dropdown.find('#idleTimeoutDetails').html(details);
      debugLog('== show enabled button');
      ...
      debugLog('== no idleTimeoutSeconds');
      dropdown.find('#idleTimeoutEnabledButton').hide();
      dropdown.find('#idleTimeoutDisabledButton').hide();
    } else {
      debugLog('== show disabled button');
      dropdown.find('#idleTimeoutEnabledButton').hide();
  ...
  function setUpdateTimeoutInfoTimeout(dropdown) {
  ...
  function updateTimeoutInfo(dropdown) {
    ...
    if (now - lastUpdateTimeoutTime > queryTimeoutInfoInterval) {
      ...
      function callback() {
        ...
        updateTimeoutDisplay(dropdown);
        setUpdateTimeoutInfoTimeout(dropdown);
      }
      function errorHandler() {
        debugLog('== xhr error response:');
        debugLog(this);
  ...
  function toggleIdleTimeout() {
    ...
    debugLog('== Changing enabled to ' + newValue);
    updateTimeoutInfo(dropdown);
    xhr(timeoutUrl, () => {
  ...
  function roundToApproximateTime(seconds) {
  ...
  function runBusyTimer() {
    debugLog('== runBusyTimer');
    clearBusyTimer();
    maybeResetIdleTimeout();
    busyTimer = window.setTimeout(runBusyTimer, kernelBusyHeartbeatInterval);
  }

  function maybeResetIdleTimeout() {
    debugLog('== request to reset idle timeout');
    now = Date.now();
  ...
  function resetIdleTimeout() {
    ...
    debugLog('== reset idle timeout');
    xhr(timeoutUrl, null, {method: 'POST'});
  }

  function notebookScrolled() {
    debugLog('== got scroll event on notebook');
    maybeResetIdleTimeout();
  }

  return {
    notebookScrolled: notebookScrolled,
    secondsToString: secondsToString, // for debugging
    setupDisconnectHandler: setupDisconnectHandler,
    setupKernelBusyHeartbeat: setupKernelBusyHeartbeat,
    setupTimeoutTimer: setupTimeoutTimer,
    toggleIdleTimeout: toggleIdleTimeout,
    updateTimeoutInfo: updateTimeoutInfo,
  };
});
